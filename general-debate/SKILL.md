...
- **Tooling/software-environment debates: gather local runtime facts at pack time** — installed version(s) of the tool under debate (including duplicate installs — check the executing binary, not just PATH), the relevant local config/settings values, and feature flags. They are one command each, they scope which bug reports and docs apply, and seats otherwise each re-derive them.
